				...
				<table id="my-grid" class="table table-striped table-bordered">
					<thead>
						<tr>
							...
							<?php if($level == 'admin' OR $level == 'master' OR $level == 'kasir' OR $level == 'spv') { ?>
							<th class='no-sort'>Registrasi</th>
							<?php } ?>
						</tr>
					</thead>
				</table>
...
if($level == 'admin' OR $level == 'master' OR $level == 'spv' OR $level == 'kasir' OR $level == 'keuangan')
{
	...
	$tambahan .= nbs(2)."<a href='".site_url('pasien/form')."' class='btn btn-default' ><i class='fa fa-plus fa-fw'></i> Import Data</a>";
...
<script type="text/javascript" language="javascript" >
	$(document).ready(function() {
		var dataTable = $('#my-grid').DataTable( {
			...
			"ajax":{
				url :"<?php echo site_url('pasien/pasien-json'); ?>",
				type: "post",
				error: function(){ 
					$(".my-grid-error").html("");
